...
const StudentIndexTable = () => {
  ...
  const { selectedStudents, selectedCohort } = useSelector(state => ({
  ...
  const onRowUpdate = (newData) => {
    dispatch(updatingStudent(newData))
  ...
  const onRowAdd = (newData) => {
    dispatch(creatingStudent({...newData, cohort_id: selectedCohort.id}))
    return new Promise((resolve) => {
      resolve()
    })
  }
